  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const { isAdminAuthorized, countOrdersByStatus, getAnalytics, getLiveFeed, todayStats, isStripeConfigured, isMetaPixelConfigured, isUtmifyConfigured, productFileExists } = lib;
